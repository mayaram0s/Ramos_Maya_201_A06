...
[https://github.com/mayaram0s/Ramos_Maya_201_A06](https://github.com/mayaram0s/Ramos_Maya_201_A06)
...
```{r}
# list of heights with same names attached as 'player_info' list
heights <- list("MJ" = 78, "LJ" = 81, "KB" = 78, "KA" = 86, "SO" = 85)

# goes through names of players to iterate through both lists
for(player in names(player_info)){
# add height of player to player_info list under same name
# '$height', element is called height
...
# iterates through names of players and adds list item
# each list item includes points per game
for(player in names(player_info)){
  points_per_game <- append(points_per_game, player_info[[player]]$points / player_info[[player]]$games_played)
}

# assigns player names to corresponding list item
...
```{r}
lapply(points_per_game, function(x){round(x, digits = 2)})
```
...
# converts each value to character, use nchar() function to count digits
  if(nchar(as.character(x)) == 2){ #if 2 digits
    return(round(x, digits = -1))

  }else if(nchar(as.character(x)) == 3){ # if 3 digits
...
apply(mat_rand, MARGIN = 1:2, round_fun)
```
...
```{r}
# only columns that include monthly max temps
month_data <- max_temps[, 3:ncol(max_temps)]

# iterates through column names
for(i in colnames(month_data)){
  print(mean(month_data[[i]])) # avg all temps under each col (month)
}
```
...
```{r}
CO_month_data <- CO_temps[, 3:ncol(CO_temps)]
medians <- apply(CO_month_data, 1, median)

which.max(medians) # returns vector index of max median
which.min(medians) # returns vector index of min median

max_year <- CO_temps[which.max(medians), 2]
min_year <- CO_temps[which.min(medians), 2]

max_median <- medians[which.max(medians)]
min_median <- medians[which.min(medians)]
...
sum_digits <- function(number){
  if(number == 0){ # base case
    ...
    last_digit <- number %% 10 # remainder when divided by 10 = last number 
    trunc_num <- number %/% 10 # takes whole number, ignores remainder
